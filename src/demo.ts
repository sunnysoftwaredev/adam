...
const createRefactorPullRequest = async (fileName: string): Promise<void> => {
    console.log(`Attempting to create a pull request for refactoring ${fileName}`);
    ...
    console.log(`✅ Created a pull request for refactoring ${fileName}`);
  };
...
export default async (): Promise<void> => {
  ...
  const filesToRefactor = files
    ...
    .slice(0, 10);
  await Promise.all(filesToRefactor.map(createRefactorPullRequest));
};
```
